...
So: prosecution, defense, and a blind judge. Four phases.

```
1. PROSECUTOR   Codex (a different model) attacks the work. Numbered charges.
2. DEFENSE      Claude answers every charge: admit, or deny with evidence.
3. JUDGE        A fresh Codex run sees charge/defense pairs with the authorship
...
State the scope in one line before proceeding ("Na ławie: `src/auth.ts`, `src/session.ts` — 340 linii zmian"). A trial over the wrong material wastes several minutes of the user's time, and the user is the only one who can catch that early.
