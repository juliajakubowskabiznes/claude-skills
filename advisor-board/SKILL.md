...
N persona subagents (N = 4 to 12, configured in `references/user-config.md`) debate a question across 3 rounds. Clarity over comfort.
...
Read [references/agent-prompts.md](references/agent-prompts.md) for the three round prompt templates. Let N = the number of board members in `user-config.md` (between 4 and 12). For each round:

1. Dispatch all N board members (in the canonical order from `user-config.md`) as subagents **in parallel** — single message, N Agent tool calls, model `sonnet`
2. Each agent writes to a temp file: `work/outputs/[debate-slug]/r[ROUND]-[name-slug].md`
3. Wait for all N confirmations. If any agent returns an ERROR (e.g. persona not found), stop and report to {{USER_NAME}} before proceeding
4. Orchestrator assembles temp files into the debate file in canonical order, then deletes the temp dir
...
[2-4 bullets — only list genuine alignment across a majority of members. Do not pad.]
...
- Board roster is fixed per `user-config.md` (N between 4 and 12) — no substitutions, no additions, no invented members during a debate
